/**
 * Frontend aesthetix form handler.
 *
 * Setup js scripts          - /inc/setup.php
 * Form handler              - /inc/addons/aesthetix/aesthetix-ajax-handler.php
 * Frontend form             - /inc/addons/aesthetix/aesthetix-frontend-form.php
 * Filter customizer options - /inc/addons/aesthetix/aesthetix-customizer.php
 * Filter root options       - /inc/addons/aesthetix/aesthetix-root-styles.php
 */

jQuery(document).ready(function ($) {

	// Click button to open/close the aesthetix form.
	$( '#footer' ).on( 'click', '#aesthetix-btn', function(e) {

		var popup  = $( '#aesthetix-popup' );
		...
		localStorage.setItem( 'aesthetixOpener', newOpener );
	...
	var aesthetixOpener = localStorage.getItem( 'aesthetixOpener' );
	if ( aesthetixOpener !== null ) {
		formOpener( aesthetixOpener );
	}

	// Open/close form function.
	function formOpener( newOpener ) {

		var newText = $( '#aesthetix-btn' ).attr( 'data-text-' + newOpener );

		$( '#aesthetix-btn' ).attr( 'data-opener', newOpener ).html( newText );
		$( '#aesthetix-popup' ).attr( 'data-opener', newOpener );
	}

	// Save project name in localStorage.
	$( '#aesthetix-name' ).change(function() {
		localStorage.setItem( 'aesthetixName', $(this).val() );
	});

	// Add title to input from localStorage.
	var aesthetixName = localStorage.getItem( 'aesthetixName' );
	if ( aesthetixName !== null ) {
		$( '#aesthetix-name' ).val(aesthetixName);
	}
	...
	function formDataSaver( data ) {
		...
		localStorage.setItem( 'aesthetixData', JSON.stringify( data ) );

		var aesthetix_value = ajax_aesthetix_obj.value;
		var obj = {};
				...
				if ( type === 'color' ) {
					...
					if ( root === 'linkColor' ) {
						...
						if ( currentValue === 'blue' ) {
							obj['linkColorDark']  = aesthetix_value['blue-600'];
							obj['linkColor']      = aesthetix_value['blue-500'];
							obj['linkColorLight'] = aesthetix_value['blue-400'];
						} else {
							var linkColorName = $( '#general-' + currentValue + '-color' ).find(':selected').val();

							obj['linkColorDark']  = aesthetix_value[ linkColorName + '-600' ];
							obj['linkColor']      = aesthetix_value[ linkColorName + '-500' ];
							obj['linkColorLight'] = aesthetix_value[ linkColorName + '-400' ];
						}
					} else {

						// General values.
						$.each( [ 50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 950 ], function( i, v ) {
							obj[ root + '-color-' + v ]  = aesthetix_value[ currentValue + '-' + v ];
						});
					}
					...
					if ( root === 'gray' ) {
						...
						// White color.
						obj['whiteColor'] = aesthetix_value[ currentValue + '-50' ];

						// Background and text color depending on the color scheme.
						if ( colorSheme === 'black' ) {
							obj['primary-bg-color']         = aesthetix_value[ currentValue + '-950' ];
							obj['primary-bg-color-hover']   = aesthetix_value[ currentValue + '-900' ];
							obj['primary-bd-color']         = aesthetix_value[ currentValue + '-800' ];
							obj['primary-bd-color-hover']   = aesthetix_value[ currentValue + '-700' ];
							obj['primary-gray-color']       = aesthetix_value[ currentValue + '-300' ];
							obj['primary-gray-color-hover'] = aesthetix_value[ currentValue + '-400' ];
							obj['primary-text-color']       = aesthetix_value[ currentValue + '-50' ];
							obj['svg-filter']               = 'invert(100%)';
						} else if ( colorSheme === 'dark' ) {
							obj['primary-bg-color']         = aesthetix_value[ currentValue + '-800' ];
							obj['primary-bg-color-hover']   = aesthetix_value[ currentValue + '-900' ];
							obj['primary-bd-color']         = aesthetix_value[ currentValue + '-900' ];
							obj['primary-bd-color-hover']   = aesthetix_value[ currentValue + '-950' ];
							obj['primary-gray-color']       = aesthetix_value[ currentValue + '-300' ];
							obj['primary-gray-color-hover'] = aesthetix_value[ currentValue + '-400' ];
							obj['primary-text-color']       = aesthetix_value[ currentValue + '-50' ];
							obj['svg-filter']               = 'invert(100%)';
						} else if ( colorSheme === 'light' ) {
							obj['primary-bg-color']         = aesthetix_value[ currentValue + '-200' ];
							obj['primary-bg-color-hover']   = aesthetix_value[ currentValue + '-300' ];
							obj['primary-bd-color']         = aesthetix_value[ currentValue + '-300' ];
							obj['primary-bd-color-hover']   = aesthetix_value[ currentValue + '-400' ];
							obj['primary-gray-color']       = aesthetix_value[ currentValue + '-500' ];
							obj['primary-gray-color-hover'] = aesthetix_value[ currentValue + '-600' ];
							obj['primary-text-color']       = aesthetix_value[ currentValue + '-950' ];
							obj['svg-filter']               = 'invert(0%)';
						} else {
							obj['primary-bg-color']         = aesthetix_value[ currentValue + '-50' ];
							obj['primary-bg-color-hover']   = aesthetix_value[ currentValue + '-200' ];
							obj['primary-bd-color']         = aesthetix_value[ currentValue + '-300' ];
							obj['primary-bd-color-hover']   = aesthetix_value[ currentValue + '-400' ];
							obj['primary-gray-color']       = aesthetix_value[ currentValue + '-500' ];
							obj['primary-gray-color-hover'] = aesthetix_value[ currentValue + '-600' ];
					...
					if ( root === 'elemBgSaturate' ) {
						...
						obj['elemBgColor'] = aesthetix_value[ elemColorName + '-' + currentValue ];

						// Elems text color.
						var elemColorStyle = getOppositeColorStyleBySaturate( currentValue );
						if ( elemColorStyle === 'white' ) {
							obj['elemTextColor'] = aesthetix_value[ elemColorName + '-50' ];
						} else {
							obj['elemTextColor'] = aesthetix_value[ elemColorName + '-900' ];
						}
						...
						var elemBdColorName  = $( '#elem-bd-color' ).find(':selected').val();
							...
							// Elems border color.
							obj['elemBdColor'] = aesthetix_value[ elemColorName + '-' + currentValue ];

							// Elems border hover color.
							if ( elemBdColorStyle === 'white' ) {
								obj['elemBdColorHover'] = aesthetix_value[ elemColorName + '-' + getPrevSaturate( currentValue ) ];
							} else {
								obj['elemBdColorHover'] = aesthetix_value[ elemColorName + '-' + getNextSaturate( currentValue ) ];
							}
						...
						obj[ root ] = aesthetix_value[ currentValue ];
					}

					// Elems shadow hover color.
					if ( root === 'elemShadow' ) {
						obj['elemShadowHover'] = aesthetix_value[ currentValue ].replace( '0.15', '0.25' );
					}
					...
					if ( root === 'btnSize' ) {
						...
						// Button size.
						obj['buttonPaddingTop'] = aesthetix_value[ currentValue ].split(' ')[0];
						obj['buttonPaddingLeft'] = aesthetix_value[ currentValue ].split(' ')[1];
					} else {
						// Other values that are converted directly.
						obj[ root ] = aesthetix_value[ currentValue ];
					}
	...
	// The function takes an array with root styles, outputs them as a string in the style#aesthetix-root tag.
	function setRootString( obj ) {
		...
		$( '#aesthetix-root' ).empty().text( rootString );
	}
	...
	// Checking locks.
	var aesthetixLocks = localStorage.getItem( 'aesthetixLocks' );

	if ( aesthetixLocks === null ) {
		aesthetixLocks = {};
	} else {
		aesthetixLocks = JSON.parse( aesthetixLocks );

		$.each(aesthetixLocks, function( index, value ) {
	...
	// Click lock.
	$( '#aesthetix-form' ).on( 'click', '.lock', function(e) {
		...
		aesthetixLocks[id] = newValue;
		localStorage.setItem( 'aesthetixLocks', JSON.stringify( aesthetixLocks ) );
	...
	$( '.selector' ).change( function(e) { 
		...
		var form = $(e.target).closest( '#aesthetix-form' );

		formDataSaver( form.serializeArray() );
	});

	// Click the button "Random".
	$( '#footer' ).on( 'click', '#aesthetix-random', function(e) {

		$( '#aesthetix-form .selector' ).each(function () {
	...
	$( '#footer' ).on( 'click', '.aesthetix-action', function(e) {
		...
		if ( ! form.hasClass( 'submited' ) ) {
			$.ajax({
				type: 'POST',
				url: ajax_aesthetix_obj.url, // ajax url (set in /inc/setup.php).
				data: {
					'action': 'aesthetix_form_action', // php event handler (/inc/addons/aesthetix/aesthetix-ajax-handler.php).
					'content': form.serialize(), // form values.
					'type': type, // form attribute.
					// 'security': ajax_aesthetix_obj.nonce, // ajax nonce (set in /inc/setup.php).
				},
				beforeSend: function () {
