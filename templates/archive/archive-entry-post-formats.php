...
if ( is_sticky() || has_post_format() ) { ?>
	<ul class="post-formats" aria-label="<?php esc_attr_e( 'Post Formats', 'aesthetix' ); ?>">
		<?php if ( is_sticky() ) { ?>
			<li <?php button_classes( 'button-small post-sticky icon icon_thumbtack', 'disabled' ) ?>>
				<?php esc_html_e( 'Sticky', 'aesthetix' ); ?>
			</li>
		<?php } ?>
		<?php if ( has_post_format() ) { ?>
			<li <?php button_classes( 'button-small post-format format format-' . get_post_format() . ' icon icon_' . get_post_format(), 'disabled' ) ?>>
