 ...
 */
if ( post_password_required() || ! get_aesthetix_options( 'general_comments_display' ) ) {
	return;
}
	...
	function aesthetix_comments_list( $comment, $args, $depth ) {
		...
		<li id="comment-<?php comment_ID(); ?>" <?php comment_class( implode( ' ', $comment_classes ) ); ?>>
			...
			<div class="comment-meta">
				...
				<div class="comment-meta__content">
					...
					<ul class="comment-meta__list">
						<li>
							<time class="comment-date data-title" datetime="<?php echo get_comment_date( 'Y-m-d\TH:i:sP' ); ?>" data-title="<?php echo esc_attr( __( 'Published Date', 'aesthetix' ) ) ?>"><?php echo get_comment_date( 'j M, Y' ) ?></time>
						</li>
						<?php if ( $edit_link = get_edit_comment_link( $comment->comment_ID ) ) : ?>
							<li>
								<a <?php link_classes(); ?> href="<?php echo esc_url( $edit_link ); ?>"><?php esc_html_e( 'Edit', 'aesthetix' ); ?></a>
							</li>
						<?php endif ?>
			...
			<?php if ( '0' == $comment->comment_approved ) { ?>
				<p class="comment-awaiting-moderation"><?php esc_html__( 'Your comment is awaiting moderation', 'aesthetix' ); ?></p>
			<?php } ?>
...
<section id="comments" class="section section_comments comments-area">
	...
	<?php if ( have_comments() ) { ?>
		...
		<h3 class="comments-title"><?php esc_html__( 'Comments:&nbsp;', 'aesthetix' ); ?><span class="count-comments"><?php comments_number( '0', '1', '%' ); ?></span></h3>
	...
	<?php if ( comments_open() ) {
		...
		$args = array(
			...
			'comment_field'        => '<div class="col-12 comment-form-comment">
				<label for="comment">' . _x( 'Comment', 'aesthetix' ) . '<span class="required">*</span></label>
		...
		<div class="no-comments">
			<p><?php _e( 'Comments are closed', 'aesthetix' ); ?></p>
		</div>
	<?php } ?>
